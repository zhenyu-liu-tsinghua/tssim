...
do
    ...
    then
        ...
        #do
            ...
            #echo "arguments       =  --preset veryslow --input ${sequence_dir}/${sequence_name} --input-res ${width}x${height} --fps ${framerate} --ref 4 --frame-thread 1 --no-wpp --bitrate ${rate} --no-pmode --no-pme --slices 0 --lookahead-slices 0 --ssim --tune ssim --qg-size 32 --aq-mode 1 --aq-strength 1.0 --bframes 8 --keyint -1 -o output.265 --csv ${seq}_${rate}.csv" >> ${sub_file}
            echo "arguments       =  --preset medium --pass 2 --input ${sequence_dir}/${sequence_name} --input-res ${width}x${height} --fps ${framerate} --ref 4 --bitrate ${rate} --ssim --tune ssim --qg-size 64 --aq-mode 1 --aq-strength 1.0 --b-adapt 2 --bframes 15 --keyint 5000 --max-tlayer 4 -o output.265 --csv ${seq}_${rate}.csv --stats ${basepath}/${seq}_${rate}/x265_2pass.log" >> ${sub_file}
            #echo "arguments       =  --preset medium --input ${sequence_dir}/${sequence_name} --input-res ${width}x${height} --fps ${framerate} --ref 4 --frame-thread 1 --no-wpp --bitrate ${rate} --no-pmode --no-pme --slices 0 --lookahead-slices 0 --ssim --tune ssim --qg-size 32 --aq-mode 1 --aq-strength 1.0 --b-adapt 1 --bframes 8 --keyint -1 -o output.265 --csv ${seq}_${rate}.csv" >> ${sub_file}
