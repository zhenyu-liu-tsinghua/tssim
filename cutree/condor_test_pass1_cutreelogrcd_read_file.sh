...
do
    ...
    then
        ...
        #do
            ...
            #veryslow ssim multi-thread
            #echo "arguments       =  --preset veryslow --min-keyint 1 --input ${sequence_dir}/${sequence_name} --input-res ${width}x${height} --fps ${framerate} --ref 4 --bitrate ${rate} --ssim --tune ssim --qg-size 64 --aq-mode 1 --aq-strength 1.0 --b-adapt 2 --bframes 15 --keyint 5000 -o output.265 --csv ${seq}_${rate}.csv" >> ${sub_file}
            #veryslow psnr single-thread
            echo "arguments       = --pass 1 --preset veryslow --min-keyint 1 --input ${sequence_dir}/${sequence_name} --input-res ${width}x${height} --fps ${framerate} --ref 4 --frame-thread 1 --no-wpp --bitrate ${rate} --no-pmode --no-pme --slices 0 --lookahead-slices 0 --psnr --tune psnr --qg-size 64 --b-adapt 2 --bframes 15 --keyint 5000 -o output-pass1.265" >> ${sub_file}
            #veryslow ssim single-thread
            #echo "arguments       =  --pass 1 --preset veryslow --min-keyint 1 --input ${sequence_dir}/${sequence_name} --input-res ${width}x${height} --fps ${framerate} --ref 4 --frame-thread 1 --no-wpp --bitrate ${rate} --no-pmode --no-pme --slices 0 --lookahead-slices 0 --ssim --tune ssim --qg-size 64 --aq-mode 1 --aq-strength 1.0 --b-adapt 2 --bframes 15 --keyint 5000 -o output-pass1.265" >> ${sub_file}
